...
from utils.tools import inject_weight_into_state, MovingAVGLinearWeightScheduler, seed_experiment, \
    get_kwargs, LinearTimeWeightScheduler


def train(config, residual=False):
    ...
    for global_step in range(number_steps):
        ...
        uncertainty = uncertainty_evaluator.get_uncertainty(state=obs_man.get_rl_state(state), action=actions_pi,
                                                            state_next=obs_man.get_rl_state(real_next_obs),
                                                            reward=rewards[0])
            ...
            for info in infos['final_info']:
                ...
                wandb.log(
                    {"rollout/episodic_return": info["episode"]["r"],
                     ...
                     "rollout/mixing_coeff_distribution": wandb.Histogram(
                         np_histogram=np.histogram(mixing_weights, bins=20, range=(0., 1.))),
                     "rollout/uncertainty_distribution_large": wandb.Histogram(
                         np_histogram=np.histogram(uncertainties, bins=20)),
                     "rollout/uncertainty_distribution_narrow": wandb.Histogram(
                         np_histogram=np.histogram(uncertainties, bins=20, range=(0, 1))),
                ...
                if eval_agent and episode_step % eval_frequency == 0 and last_evaluated_episode != episode_step and global_step >= \
                    ...
                    tracked_returns = [evaluator.evaluate_agent_on_start_states(agent_trainer.agent, agent_type=Algorithm.RL, weight=eval_weights[0])]
                    ...
                    print("Performance for eval weight: ", adapted_return, eval_max_return)
                    if adapted_return > eval_max_return:
                        eval_max_return = adapted_return
                        logger.save_model(agent_trainer.agent)
                        if capture_video:
                            frames = evaluator.collect_video_frames(agent_trainer.agent, agent_type=Algorithm.RL, weight=eval_weights[-1],
        ...
        if residual:
            ...
            agent_trainer.add_to_replay_buffer(obs, real_next_obs, actions, rewards, terminations, infos)
