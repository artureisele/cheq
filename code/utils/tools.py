...
from custom_envs.CarEnv.Utils import load_tracks
...
def make_racing_env(seed, no_training_tracks=None, training_tracks_path=None):

    def thunk():
        if training_tracks_path is not None:
            tracks = load_tracks(training_tracks_path)
        elif no_training_tracks is not None:
            rng = np.random.default_rng(seed=seed)
            tracks = [make_full_environment(width=RACING_FAST['problem']['track_width'],
                                                     extends=(
                                                     RACING_FAST['problem']['extend'], RACING_FAST['problem']['extend']),
                                                     cone_width=RACING_FAST['problem']['cone_width'],
                                                     rng=rng) for _ in range(no_training_tracks)]
        else:
            tracks = None
        env = gym.make('CarEnv-v1', training_tracks=tracks)
        env = gym.wrappers.FlattenObservation(env)
        env = gym.wrappers.RecordEpisodeStatistics(env)
        env.reset(seed=seed)
        env.action_space.seed(seed)
